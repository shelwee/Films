...
{ 
 ...
 Sdate=Sdate.split("-");
 Calendar_Show(parseInt(Sdate[0]),parseInt(Sdate[1],10),parseInt(Sdate[2],10));
}
function Calendar_GetDate(y,m,d)
{
var s_m = m+"";
...
if(!Calendar_obj2||d!=0){
 if(d>0){Calendar_obj.value=y+'-'+s_m+'-'+s_d;}
 else{
  var objEvent=divCalendar.event||arguments.callee.caller.arguments[0];
  var srcElement=objEvent.srcElement;
  if(!srcElement){srcElement=objEvent.target;}
  s_d = srcElement.innerHTML;
  if(s_d.length == 1){
	s_d = "0"+s_d;
	}
  Calendar_obj.value=y+'-'+s_m+'-'+s_d;
 }
}
else{
